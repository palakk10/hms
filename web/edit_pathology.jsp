...
<%
    ...
    String pathologyId = request.getParameter("pathology_id");
    ...
    // Input validation
    if (pathologyId == null || patientId == null || caseId == null || testId == null || testDate == null ||
        ...
        pathologyId.trim().isEmpty() || patientId.trim().isEmpty() || caseId.trim().isEmpty() || 
        testId.trim().isEmpty() || testDate.trim().isEmpty()) {
    ...
    try {
        ...
        // Parse inputs
        int pathologyIdInt = Integer.parseInt(pathologyId);
        ...
        // Validate pathology ID
        ps = conn.prepareStatement("SELECT PATHOLOGY_ID FROM pathology WHERE PATHOLOGY_ID = ?");
        ps.setInt(1, pathologyIdInt);
        rs = ps.executeQuery();
        if (!rs.next()) {
            throw new Exception("Invalid Pathology ID.");
        }
        rs.close();
        ps.close();
        ...
        ps = conn.prepareStatement(
            "UPDATE pathology SET ID = ?, CASE_ID = ?, TEST_ID = ?, TEST_DATE = ?, B_TEST = ?, BT_COUNT = ?, " +
        ...
        ps.setInt(17, pathologyIdInt);

        int rows = ps.executeUpdate();
        if (rows > 0) {
            conn.commit();
            success = true;
            out.println("<script>alert('Pathology record updated successfully!'); window.location='receptionist.jsp?status=success';</script>");
        } else {
            conn.rollback();
            out.println("<script>alert('Error: Failed to update pathology record.'); window.location='receptionist.jsp?status=error';</script>");
        }
